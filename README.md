...
- `source` (not recommended for production): compiles the in-repo bootstrap bridge target (`Sources/LibtorrentAppleBridge`) for API development and fast iteration.
  ...
  A concrete drift example from earlier versions: binary-mode `applyConfiguration` was gated off in Swift wrapper even though the native bridge already supported runtime rate-limit apply; this is already fixed and included in `0.2.3`, but it shows why source and binary parity must be validated on binary mode.
...
Run the local benchmark demo (v0.2.3 P0-0):
